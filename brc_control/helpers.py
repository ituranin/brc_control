...
def _find_center_at(mask, prediction, perp, search_width, grid_step, r=10):
    h, w = mask.shape
    best_score = -1
    best_point = None
    ...
    if len(xs):
        return np.array([x0 + xs.mean(), y0 + ys.mean()])
    else:
        return best_point
...
def follow_centerline_grid(mask, start_xy, step=15, search_width=50, grid_step=10, max_steps=100):
    ...
    # snap the first point sideways onto the path, no forward movement
    perp = np.array([-direction[1], direction[0]])
    snapped = _find_center_at(mask, center, perp, search_width, grid_step)
    ...
    for _ in range(max_steps):
        ...
        new_point = _find_center_at(mask, prediction, perp, search_width, grid_step)
...
def steering_point_from_topdown(track, distance=32, debug=False):
    ...
    center_points_t = follow_centerline_grid(center_line, (384//2, 384-1), max_steps=10, search_width=40, step=20, grid_step=10)
    degree = 3
...
def speed_from_topdown(sensor: VirtualDistanceSensor, img):
    ...
    l = len(dists)
    mid = l // 2
    dist = np.min(dists[mid]) - 7.0
    speed = calculate_max_target_speed(max(dist, 0.0), 4.5, (3.0*3.6))
